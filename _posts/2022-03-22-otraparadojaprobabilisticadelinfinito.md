...
<br/>
Escojamos un número entero positivo $\mathfrak{x}\in\mathbb{Z}_+$ al azar de modo que la probabilidad de elegir $n\in\mathbb{Z}_+$ sea
...
donde $\log$ es el logaritmo en base 10 y $\lfloor~\rfloor$ denota que redondeamos hacia abajo, esto es, $\lfloor \log(\mathfrak{x})\rfloor$ es $\log(\fkx)$ omitiendo los decimales tras la coma.
...
Por un lado, hemos visto que el valor esperado de $\mathfrak{x}$ es infinito. Por otro lado, la cantidad esperada de dígitos de $\mathfrak{x}$ es a lo sumo dos. Así, hemos llegado a la siguiente 'paradoja'[^petersurgo]:
